...
# Load dataset
file_path = '/Users/Ramadhan/NP-MTL/Filtering_Preprocessing/protease_three_groups_final_dataset_for_stl_mtl.txt'
...
# Group by Target Parent ID, Protein Class ID, and Target ChEMBL ID
groups = data.groupby(['Target Parent ID', 'Protein Class ID', 'Target ChEMBL ID'])

# Process nested CV for each group separately using pre-assigned global folds
for (target_parent_id, protein_class_id, target_chembl_id), group in groups:
    # Prepare feature matrix X and target vector y for the group
    X_group = np.stack(group['Avalon_FP'].values)
    ...
    for fold in group_global_folds:
        ...
        best_params_dict[(target_parent_id, protein_class_id, target_chembl_id, fold)] = best_params
...
predictions_save_path = '/Users/Ramadhan/NP-MTL/STL_MTL_Three_Groups_Studies/protease_three_groups_rf_stl_prediction.txt'
data.to_csv(predictions_save_path, sep='\t', index=False)
...
final_results.columns = [
    ...
    'STL Average RMSE', 'STL RMSE SD'
]

# Save the statistic file
final_results_save_path = '/Users/Ramadhan/NP-MTL/STL_MTL_Three_Groups_Studies/protease_three_groups_rf_stl_statistics.txt'
