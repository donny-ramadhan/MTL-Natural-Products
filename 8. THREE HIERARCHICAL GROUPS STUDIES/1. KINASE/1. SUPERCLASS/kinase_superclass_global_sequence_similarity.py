...
# Extract unique UniProt Accession IDs
uniprot_ids = df['UniProt Accession'].unique()
...
# Convert similarity_matrix to DataFrame
df_sim = pd.DataFrame(similarity_matrix)
max_size = df_sim.shape[1]
...
for idx, row in df.iterrows():
    ...
    if accession in df_sim.columns:
        sim_values = df_sim.loc[accession].tolist()
        average_similarity = calculate_average_similarity_above_diagonal(df_sim)
...
output_path = '/Users/Ramadhan/NP-MTL/Filtering_Preprocessing/kinase_three_groups_superclass_final_dataset_gss_for_stl_mtl.txt'
