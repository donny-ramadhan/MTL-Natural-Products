...
# Load data
data_path = '/Users/Ramadhan/NP-MTL/Filtering_Preprocessing/kinase_three_groups_final_dataset_for_stl_mtl.txt'
df = pd.read_csv(data_path, sep='\t', low_memory=False)

# Extract unique combinations of 'Target Parent ID' and 'Protein Class ID' and corresponding 'UniProt Accession'
grouped = df.groupby(['Target Parent ID', 'Protein Class ID'])['UniProt Accession'].unique()
...
# Process each group
for (target_parent_id, protein_class_id), uniprot_ids in grouped.items():
    ...
    similarity_matrices[(target_parent_id, protein_class_id)] = df_sim
...
for (tpid, pcid), matrix in similarity_matrices.items():
    ids = matrix.columns
    for idx, row in df[(df['Target Parent ID'] == tpid) & (df['Protein Class ID'] == pcid)].iterrows():
...
output_path = '/Users/Ramadhan/NP-MTL/Filtering_Preprocessing/kinase_three_groups_protein_class_final_dataset_lss_for_stl_mtl.txt'
